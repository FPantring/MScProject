...
# When running this script with real data instead of simulated, instead of the ground truth map a tap seperated file should be uploaded,
...
# in the second column in the format R>V, with R being the reference allele at this position and V the variant allele.  
# Sor for example: python3 Pileup_Processing.py T_WGS.bam GRCh38.d1.vd1.fa WGS_gtMap.txt > output.txt
...
# create empty dictornaries that will be filled in later loops
poslist = []
refdict = {}
alleledict = {}
#df = pd.DataFrame(columns=["init", "new", "cigar"])
appended_data = {}
...
def base_pos(l, startpos, cigar, orig):
    ...
    # to get this position it is necessary to substract the start position of the read alignment to the reference
    ...
    if re.findall('I|D|S', cigar) != []: # check if insertion or deletion present
        start = 0
        dl = []
        # seperate the string at the number before a D or I to extract all deletions and insertions:
        for num1, i_or_d in re.findall('(\d+)([IDSM])', cigar):
            ...
            if num1:
                start += int(num1)
        for j in dl:
            # if the indel occured after the loci in question, no adjustment of the position is necessary)
            if distance < j[2]:
                continue
            # if a region in the beginning is sofclipped, this is likely a result of the vairant in the read,
            ...
            # if the indel occured before the loci in question (or at the loci in the case of an insertion), 
            # adjust the position number:
            else:
                # in case of an insertion, substract the number of inserted bases from the current position 
                ...
                if j[1] == "I":
                    if orig == False:
                        ...
                        distance = distance + int(j[0])
                # in case of a deletion, add the number of deleted bases to the current position when adjuasting for an alternative location
                elif j[1] == "D":
                    if orig == False:
                        distance = distance + int(j[0])
                    else: # When adjusting for the original CIGAR string, substract the number of deleted bases
                        distance = distance - int(j[0])
    if cigar != "76M":
...
with open(sys.argv[3]) as gtMap:
    ...
    for line in csv.reader(gtMap, dialect="excel-tab"):
        ...
        # access the read pileup at a specific postion:
        # REMEMBER: take (position-1, position) instead of (position, position+1), because coordinates are 0-based
        # access the alternative hits with .get_tag("XA") and concatenate all alterative hits to one list
        ...
        # the inital location of the read in the format chr:positionnumber, the read sequence and the CIGAR string of the original alignment
        element=[re.sub("[+-]", "", s) + "," + str(base_pos(l = (int(loc[1]) - 1), startpos = pileupread.alignment.reference_start,
                 ...
                 # check whether the position in question has no alternative aligments in the genome
...
# Convert the list into a dataframe where the information is seperated into columns
...
def revcheck(i):
    # it is important to adjust the position according to the CIGAR string, because if some bases are anotated as soft clipped
...
# create an additional column in the dataframe, which includes the actual variant position:
...
# apply the function to the dataframe of unique rows       
